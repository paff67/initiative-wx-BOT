...
| `persona.yaml` | 人设结构化配置。 |
| `voice.md` | 长文本 voice/persona 渲染规范。 |
...
| --- | ---: | ---: | --- |
| `micro_send` | 30 min | 2 | false |
| `closure` | 60 min | 2 | false |
| `random_share` | 120 min | 2 | true |
| `care_timing` | 120 min | 2 | true |
| `normal_send` | 180 min | 3 | true |
...
- 不作为最终用户可见说话者
- 不硬编码具体人设
- persona 和 voice 由 profile config 注入
- 输出 JSON
...
- 当前启用 `weather_openmeteo.get_current_weather`，采集上海当前天气，带 TTL 缓存。
- 不调用 LLM。
...
### 12.5 Conversation Ledger 与语音记录
...
| `POST /api/preview/full` | full dry-run preview，不投递。 |
...
## 14. Config Edit / Backup / Rollback
...
curl -sS -X POST http://127.0.0.1:8790/api/preview/full \
  -H 'Content-Type: application/json' \
  -d '{"profile_id":"linjiang","mode":"full","force_llm":true}' | python3 -m json.tool
```
...
curl -sS -X POST http://127.0.0.1:8790/api/preview/full \
  -H 'Content-Type: application/json' \
  -d '{"profile_id":"testprofile","mode":"full","force_llm":true}' | python3 -m json.tool
```
